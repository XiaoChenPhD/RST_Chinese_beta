...
let expName = 'rumination';  // from the Builder filename that created this script
let expInfo = {
    'participant': '999',
    'session': '001',
};
...
async function experimentInit() {
  ...
  image_emotional_instruct = new visual.ImageStim({
    win : psychoJS.window,
    name : 'image_emotional_instruct', units : undefined, 
...
function ruminationRoutineEnd(snapshot) {
  return async function () {
    ...
    if (routineForceEnded) {
        ...
        ruminationClock.add(120.000000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
...
function emotion_instructRoutineBegin(snapshot) {
  return async function () {
    ...
    emotion_instructComponents.push(image_emotional_instruct);
...
function emotion_instructRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && image_emotional_instruct.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      image_emotional_instruct.tStart = t;  // (not accounting for frame time here)
    ...
    if (image_emotional_instruct.status === PsychoJS.Status.STARTED) {
    }
