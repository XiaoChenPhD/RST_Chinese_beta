...
let expName = 'sad_memory_demo';  // from the Builder filename that created this script
let expInfo = {
    'participant': '999',
    'session': '001',
};
...
const sad_memory_loopLoopScheduler = new Scheduler(psychoJS);
flowScheduler.add(sad_memory_loopLoopBegin(sad_memory_loopLoopScheduler));
flowScheduler.add(sad_memory_loopLoopScheduler);
flowScheduler.add(sad_memory_loopLoopEnd);
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'think_content_condition_task.xlsx', 'path': 'think_content_condition_task.xlsx'},
    {'name': 'stimuli/sad_memo_instruct.png', 'path': 'stimuli/sad_memo_instruct.png'},
...
async function experimentInit() {
  ...
  image_sad_memo_instruct = new visual.ImageStim({
    win : psychoJS.window,
    name : 'image_sad_memo_instruct', units : undefined, 
    image : 'stimuli/sad_memo_instruct.png', mask : undefined,
    anchor : 'center',
    ori : 0.0, 
    pos : [0, 0], 
    draggable: false,
  ...
  file_name = `dysphoric_memory_keywords/subject_${subject_id}.xlsx`;
  ...
  sad_memoryClock = new util.Clock();
  background = new visual.TextStim({
    win: psychoJS.window,
    name: 'background',
    text: '',
    font: 'Noto Sans SC',
...
function welcome_screenRoutineBegin(snapshot) {
  return async function () {
    ...
    welcome_screenComponents.push(image_sad_memo_instruct);
...
function welcome_screenRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && image_sad_memo_instruct.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      image_sad_memo_instruct.tStart = t;  // (not accounting for frame time here)
    ...
    if (image_sad_memo_instruct.status === PsychoJS.Status.STARTED) {
    }
    ...
    if (key_resp_welcome.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_welcome.getKeys({keyList: ['k','l'], waitRelease: false});
...
function wait4trigger_scannerRoutineBegin(snapshot) {
  return async function () {
    ...
    // keep track of whether this Routine was forcibly ended
    routineForceEnded = false;
    wait4trigger_scannerClock.reset(routineTimer.getTime());
    ...
    for (const thisComponent of wait4trigger_scannerComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function wait4trigger_scannerRoutineEachFrame() {
  return async function () {
    ...
    if (wait4trigger_text.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      // keep track of stop time/frame for later
      wait4trigger_text.tStop = t;  // not accounting for scr refresh
      wait4trigger_text.frameNStop = frameN;  // exact frame index
      // update status
      wait4trigger_text.status = PsychoJS.Status.FINISHED;
...
function wait4trigger_scannerRoutineEnd(snapshot) {
  return async function () {
    ...
    if (routineForceEnded) {
        routineTimer.reset();} else if (wait4trigger_scannerMaxDurationReached) {
        wait4trigger_scannerClock.add(wait4trigger_scannerMaxDuration);
    } else {
        wait4trigger_scannerClock.add(1.000000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
...
function fake_stimulusRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of fake_stimulusComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function fake_stimulusRoutineEnd(snapshot) {
  return async function () {
    ...
    if (routineForceEnded) {
        ...
        fake_stimulusClock.add(2.000000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
...
var sad_memory_loop;
function sad_memory_loopLoopBegin(sad_memory_loopLoopScheduler, snapshot) {
  return async function() {
    TrialHandler.fromSnapshot(snapshot); // update internal variables (.thisN etc) of the loop
    
    // set up handler to look after randomisation of conditions etc
    sad_memory_loop = new TrialHandler({
      psychoJS: psychoJS,
      nReps: 1, method: TrialHandler.Method.RANDOM,
      extraInfo: expInfo, originPath: undefined,
      trialList: file_name,
      seed: undefined, name: 'sad_memory_loop'
    });
    psychoJS.experiment.addLoop(sad_memory_loop); // add the loop to the experiment
    currentLoop = sad_memory_loop;  // we're now the current loop
    
    // Schedule all the trials in the trialList:
    for (const thisSad_memory_loop of sad_memory_loop) {
      snapshot = sad_memory_loop.getSnapshot();
      sad_memory_loopLoopScheduler.add(importConditions(snapshot));
      sad_memory_loopLoopScheduler.add(sad_memoryRoutineBegin(snapshot));
      sad_memory_loopLoopScheduler.add(sad_memoryRoutineEachFrame());
      sad_memory_loopLoopScheduler.add(sad_memoryRoutineEnd(snapshot));
      sad_memory_loopLoopScheduler.add(sad_memory_loopLoopEndIteration(sad_memory_loopLoopScheduler, snapshot));
    }
...
async function sad_memory_loopLoopEnd() {
  // terminate loop
  psychoJS.experiment.removeLoop(sad_memory_loop);
...
function sad_memory_loopLoopEndIteration(scheduler, snapshot) {
...
function think_content_loopLoopBegin(think_content_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    think_content_loop = new TrialHandler({
      ...
      extraInfo: expInfo, originPath: undefined,
      trialList: 'think_content_condition_task.xlsx',
    ...
    // Schedule all the trials in the trialList:
    for (const thisThink_content_loop of think_content_loop) {
...
var sad_memoryMaxDurationReached;
var counter;
var text_height;
var pos1;
var pos2;
...
var keyword3_text;
var keyword4_text;
var sad_memoryMaxDuration;
var sad_memoryComponents;
function sad_memoryRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    ...
    // keep track of whether this Routine was forcibly ended
    routineForceEnded = false;
    sad_memoryClock.reset(routineTimer.getTime());
    routineTimer.add(3.000000);
    sad_memoryMaxDurationReached = false;
    // update component parameters for each repeat
    background.setText('');
    // Run 'Begin Routine' code from code
    counter = 0;
    if ((((typeof keyword1) === "string") || (keyword1 instanceof String))) {
        counter = (counter + 1);
    }
    if ((((typeof keyword2) === "string") || (keyword2 instanceof String))) {
        counter = (counter + 1);
    }
    if ((((typeof keyword3) === "string") || (keyword3 instanceof String))) {
        counter = (counter + 1);
    }
    if ((((typeof keyword4) === "string") || (keyword4 instanceof String))) {
        counter = (counter + 1);
    }
    text_height = 0.1;
    ...
    if ((counter === 1)) {
        keyword1_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword1_text", "text": keyword1, "font": "Noto Sans SC", "pos": pos1[0], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
        keyword2_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword2_text", "text": "", "font": "Noto Sans SC", "pos": pos1[1], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
        keyword3_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword3_text", "text": "", "font": "Noto Sans SC", "pos": pos1[2], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
        keyword4_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword4_text", "text": "", "font": "Noto Sans SC", "pos": pos1[3], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
    } else {
        if ((counter === 2)) {
            keyword1_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword1_text", "text": keyword1, "font": "Noto Sans SC", "pos": pos2[0], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
            keyword2_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword2_text", "text": keyword2, "font": "Noto Sans SC", "pos": pos2[1], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
            keyword3_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword3_text", "text": "", "font": "Noto Sans SC", "pos": pos2[2], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
            keyword4_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword4_text", "text": "", "font": "Noto Sans SC", "pos": pos2[3], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
        } else {
            if ((counter === 3)) {
                keyword1_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword1_text", "text": keyword1, "font": "Noto Sans SC", "pos": pos3[0], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
                keyword2_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword2_text", "text": keyword2, "font": "Noto Sans SC", "pos": pos3[1], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
                keyword3_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword3_text", "text": keyword3, "font": "Noto Sans SC", "pos": pos3[2], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
                keyword4_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword4_text", "text": "", "font": "Noto Sans SC", "pos": pos3[3], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
            } else {
                if ((counter === 4)) {
                    keyword1_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword1_text", "text": keyword1, "font": "Noto Sans SC", "pos": pos4[0], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
                    keyword2_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword2_text", "text": keyword2, "font": "Noto Sans SC", "pos": pos4[1], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
                    keyword3_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword3_text", "text": keyword3, "font": "Noto Sans SC", "pos": pos4[2], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
                    keyword4_text = new visual.TextStim({"win": psychoJS.window, "name": "keyword4_text", "text": keyword4, "font": "Noto Sans SC", "pos": pos4[3], "height": text_height, "wrapWidth": null, "ori": 0.0, "color": "white", "colorSpace": "rgb", "opacity": null, "languageStyle": "LTR", "depth": 0.0});
                }
            }
        }
    }
    ...
    psychoJS.experiment.addData('sad_memory.started', globalClock.getTime());
    sad_memoryMaxDuration = null
    // keep track of which components have finished
    sad_memoryComponents = [];
    sad_memoryComponents.push(background);
    
    for (const thisComponent of sad_memoryComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function sad_memoryRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'sad_memory' ---
    // get current time
    t = sad_memoryClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    ...
    if (background.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      ...
      // update status
      background.status = PsychoJS.Status.FINISHED;
...
function sad_memoryRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('sad_memory.stopped', globalClock.getTime());
    ...
    if (routineForceEnded) {
        routineTimer.reset();} else if (sad_memoryMaxDurationReached) {
        sad_memoryClock.add(sad_memoryMaxDuration);
    } else {
        sad_memoryClock.add(3.000000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
...
function emotion_instructRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emotion_instructComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emotion_instructRoutineEachFrame() {
  return async function () {
    ...
    if (key_resp_emotion_instruct.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_emotion_instruct.getKeys({keyList: ['k','l'], waitRelease: false});
...
function emotional_levelRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emotional_levelComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emotional_levelRoutineEachFrame() {
  return async function () {
    ...
    if (key_resp_emotion.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_emotion.getKeys({keyList: ['k','l'], waitRelease: false});
...
function think_content_instructRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of think_content_instructComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function think_content_instructRoutineEachFrame() {
  return async function () {
    ...
    if (key_resp_think_content_instruct.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_think_content_instruct.getKeys({keyList: ['k','l'], waitRelease: false});
...
function think_contentRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of think_contentComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function think_contentRoutineEachFrame() {
  return async function () {
    ...
    if (key_resp_thinking_content.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_thinking_content.getKeys({keyList: ['k','l'], waitRelease: false});
...
function goodbyeRoutineEachFrame() {
  return async function () {
    ...
    if (key_resp_goodbye.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_goodbye.getKeys({keyList: ['k','l'], waitRelease: false});
