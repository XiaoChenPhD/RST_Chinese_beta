...
let expName = 'rumination_demo';  // from the Builder filename that created this script
let expInfo = {
    'participant': '999',
    'session': '001',
};
...
const rumination_loopLoopScheduler = new Scheduler(psychoJS);
flowScheduler.add(rumination_loopLoopBegin(rumination_loopLoopScheduler));
flowScheduler.add(rumination_loopLoopScheduler);
flowScheduler.add(rumination_loopLoopEnd);
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'rum_prompt_image.xlsx', 'path': 'rum_prompt_image.xlsx'},
    {'name': 'stimuli/rum_prompt_1.png', 'path': 'stimuli/rum_prompt_1.png'},
    {'name': 'stimuli/rum_prompt_2.png', 'path': 'stimuli/rum_prompt_2.png'},
    {'name': 'stimuli/rum_prompt_3.png', 'path': 'stimuli/rum_prompt_3.png'},
    {'name': 'stimuli/rum_prompt_4.png', 'path': 'stimuli/rum_prompt_4.png'},
    {'name': 'think_content_condition_task.xlsx', 'path': 'think_content_condition_task.xlsx'},
    {'name': 'stimuli/rum_instruct.png', 'path': 'stimuli/rum_instruct.png'},
...
async function experimentInit() {
  ...
  image_rum_instruct = new visual.ImageStim({
    win : psychoJS.window,
    name : 'image_rum_instruct', units : undefined, 
    image : 'stimuli/rum_instruct.png', mask : undefined,
    anchor : 'center',
    ori : 0.0, 
    pos : [0, 0], 
    draggable: false,
  ...
  ruminationClock = new util.Clock();
  image_rum = new visual.ImageStim({
    win : psychoJS.window,
    name : 'image_rum', units : undefined, 
...
function welcome_screenRoutineBegin(snapshot) {
  return async function () {
    ...
    welcome_screenComponents.push(image_rum_instruct);
...
function welcome_screenRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && image_rum_instruct.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      image_rum_instruct.tStart = t;  // (not accounting for frame time here)
    ...
    if (image_rum_instruct.status === PsychoJS.Status.STARTED) {
    }
...
var rumination_loop;
function rumination_loopLoopBegin(rumination_loopLoopScheduler, snapshot) {
  return async function() {
    TrialHandler.fromSnapshot(snapshot); // update internal variables (.thisN etc) of the loop
    
    // set up handler to look after randomisation of conditions etc
    rumination_loop = new TrialHandler({
      psychoJS: psychoJS,
      nReps: 1, method: TrialHandler.Method.RANDOM,
      extraInfo: expInfo, originPath: undefined,
      trialList: 'rum_prompt_image.xlsx',
      seed: undefined, name: 'rumination_loop'
    });
    psychoJS.experiment.addLoop(rumination_loop); // add the loop to the experiment
    currentLoop = rumination_loop;  // we're now the current loop
    
    // Schedule all the trials in the trialList:
    for (const thisRumination_loop of rumination_loop) {
      snapshot = rumination_loop.getSnapshot();
      rumination_loopLoopScheduler.add(importConditions(snapshot));
      rumination_loopLoopScheduler.add(ruminationRoutineBegin(snapshot));
      rumination_loopLoopScheduler.add(ruminationRoutineEachFrame());
      rumination_loopLoopScheduler.add(ruminationRoutineEnd(snapshot));
      rumination_loopLoopScheduler.add(rumination_loopLoopEndIteration(rumination_loopLoopScheduler, snapshot));
    }
...
async function rumination_loopLoopEnd() {
  // terminate loop
  psychoJS.experiment.removeLoop(rumination_loop);
...
function rumination_loopLoopEndIteration(scheduler, snapshot) {
...
var ruminationMaxDurationReached;
var ruminationMaxDuration;
var ruminationComponents;
function ruminationRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    ...
    // keep track of whether this Routine was forcibly ended
    routineForceEnded = false;
    ruminationClock.reset(routineTimer.getTime());
    routineTimer.add(5.000000);
    ruminationMaxDurationReached = false;
    // update component parameters for each repeat
    image_rum.setImage(prompt_image);
    psychoJS.experiment.addData('rumination.started', globalClock.getTime());
    ruminationMaxDuration = null
    // keep track of which components have finished
    ruminationComponents = [];
    ruminationComponents.push(image_rum);
    
    for (const thisComponent of ruminationComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function ruminationRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'rumination' ---
    // get current time
    t = ruminationClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    
    // *image_rum* updates
    if (t >= 0.0 && image_rum.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      image_rum.tStart = t;  // (not accounting for frame time here)
    ...
    if (image_rum.status === PsychoJS.Status.STARTED) {
    }
    
    frameRemains = 0.0 + 5 - psychoJS.window.monitorFramePeriod * 0.75;// most of one frame period left
    if (image_rum.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      // keep track of stop time/frame for later
      image_rum.tStop = t;  // not accounting for scr refresh
      image_rum.frameNStop = frameN;  // exact frame index
      // update status
      image_rum.status = PsychoJS.Status.FINISHED;
...
function ruminationRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('rumination.stopped', globalClock.getTime());
    if (routineForceEnded) {
        routineTimer.reset();} else if (ruminationMaxDurationReached) {
        ruminationClock.add(ruminationMaxDuration);
    } else {
        ruminationClock.add(5.000000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
