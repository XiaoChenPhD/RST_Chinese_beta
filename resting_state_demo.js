...
let expName = 'resting_state_demo';  // from the Builder filename that created this script
let expInfo = {
    'participant': '999',
    'session': '001',
};
...
async function experimentInit() {
  ...
  key_resp_rest_instruct = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
...
function welcomeRoutineEachFrame() {
  return async function () {
    ...
    // if key_welcome is active this frame...
    if (key_welcome.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_welcome.getKeys({keyList: ['k','l'], waitRelease: false});
...
var _key_resp_rest_instruct_allKeys;
...
function rest_instructRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    key_resp_rest_instruct.keys = undefined;
    key_resp_rest_instruct.rt = undefined;
    ...
    rest_instructComponents.push(key_resp_rest_instruct);
    
    for (const thisComponent of rest_instructComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function rest_instructRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && key_resp_rest_instruct.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      key_resp_rest_instruct.tStart = t;  // (not accounting for frame time here)
      key_resp_rest_instruct.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { key_resp_rest_instruct.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { key_resp_rest_instruct.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { key_resp_rest_instruct.clearEvents(); });
    }
    
    // if key_resp_rest_instruct is active this frame...
    if (key_resp_rest_instruct.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_rest_instruct.getKeys({keyList: ['k','l'], waitRelease: false});
      _key_resp_rest_instruct_allKeys = _key_resp_rest_instruct_allKeys.concat(theseKeys);
      if (_key_resp_rest_instruct_allKeys.length > 0) {
        key_resp_rest_instruct.keys = _key_resp_rest_instruct_allKeys[_key_resp_rest_instruct_allKeys.length - 1].name;  // just the last key pressed
        key_resp_rest_instruct.rt = _key_resp_rest_instruct_allKeys[_key_resp_rest_instruct_allKeys.length - 1].rt;
        key_resp_rest_instruct.duration = _key_resp_rest_instruct_allKeys[_key_resp_rest_instruct_allKeys.length - 1].duration;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function rest_instructRoutineEnd(snapshot) {
  return async function () {
    ...
    // update the trial handler
    if (currentLoop instanceof MultiStairHandler) {
      currentLoop.addResponse(key_resp_rest_instruct.corr, level);
    }
    psychoJS.experiment.addData('key_resp_rest_instruct.keys', key_resp_rest_instruct.keys);
    if (typeof key_resp_rest_instruct.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('key_resp_rest_instruct.rt', key_resp_rest_instruct.rt);
        psychoJS.experiment.addData('key_resp_rest_instruct.duration', key_resp_rest_instruct.duration);
        routineTimer.reset();
        }
    
    key_resp_rest_instruct.stop();
